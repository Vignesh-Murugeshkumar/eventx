...
<body class="min-h-full bg-slate-950 text-slate-200">
  ...
  // --------------------
  // Data Models (local caches, populated/synced with Firestore)
  // --------------------
  // Note: session still uses localStorage, but users and events now sync with Firestore
  ...
  const db = { events: [] }; // Events will be loaded from Firestore
  let users = []; // Users will be loaded from Firestore
  let session = loadLocal(SESSION_KEY, null); // Session remains local for simplicity
  ...
  // Fetches all events from Firestore and updates the local db.events array
  async function loadEventsFromFirestore() {
    if (!window.dbFirestore) return;
    try {
      const eventsCol = window.firebaseCollection(window.dbFirestore, "events");
      const querySnap = await window.firebaseGetDocs(eventsCol);
      db.events = []; // Clear local events
      querySnap.forEach(doc => db.events.push(doc.data()));
      render(); // Re-render after loading data
    } catch (error) {
      console.error("Error loading events from Firestore:", error);
      setToast("Failed to load events.", false);
    }
  }

  // Fetches all users from Firestore and updates the local users array
  async function loadUsersFromFirestore() {
    if (!window.dbFirestore) return;
    try {
      const usersCol = window.firebaseCollection(window.dbFirestore, "users");
      const querySnap = await window.firebaseGetDocs(usersCol);
      users = []; // Clear local users
      querySnap.forEach(doc => users.push(doc.data()));
    } catch (error) {
      console.error("Error loading users from Firestore:", error);
      setToast("Failed to load users.", false);
    }
  }

  // Seeds data if collections are empty (for first run demo)
  async function seedFirestoreData() {
    if (!window.dbFirestore) return;

    // Check if users collection is empty
    const usersCol = window.firebaseCollection(window.dbFirestore, "users");
    const usersSnap = await window.firebaseGetDocs(usersCol);
    if (usersSnap.empty) {
      const organiserUser = { email:'organiser@example.com', password:'organiser123', role:'organiser', name:'Demo Organiser', clubName: 'Tech Club' };
      const studentUser = { email:'student@example.com', password:'student123', role:'student', name:'Demo Student' };
      await window.firebaseSetDoc(window.firebaseDoc(usersCol, organiserUser.email), organiserUser);
      await window.firebaseSetDoc(window.firebaseDoc(usersCol, studentUser.email), studentUser);
      console.log("Seeded demo users.");
    }

    // Check if events collection is empty
    const eventsCol = window.firebaseCollection(window.dbFirestore, "events");
    const eventsSnap = await window.firebaseGetDocs(eventsCol);
    if (eventsSnap.empty) {
      const eId = uid();
      const demoEvent = {
        id:eId,
      ...
      await window.firebaseSetDoc(window.firebaseDoc(eventsCol, eId), demoEvent);
      console.log("Seeded demo event.");
    }
  }

  function load(collection, callback) {
    // Realtime listener
    return window.firebaseCollection(window.dbFirestore, collection).onSnapshot(snapshot => {
      const data = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
      // Update localStorage for offline support
      localStorage.setItem(collection, JSON.stringify(data));
      // Send back to your UI
  ...
  // --------------------
  // Auth (Custom using Firestore 'users' collection)
  // --------------------
  async function signUp({email,password,role,name,clubName}){
    ...
    try {
      await window.firebaseSetDoc(window.firebaseDoc(window.firebaseCollection(window.dbFirestore, "users"), email), newUser);
  ...
  async function signIn({email,password}){
    try {
      const userDocRef = window.firebaseDoc(window.firebaseCollection(window.dbFirestore, "users"), email);
      const userSnap = await window.firebaseGetDocs(userDocRef); // Use getDocs for a specific doc
      const u = userSnap.data(); // Get the document data
  ...
  window.onRegister = async function(role){
    try {
  ...
  window.mockPay = async function(eventId, method){
    ...
    e.participants.push(reg); // Add to local cache first
    e.updated_at = new Date().toISOString();

    try {
      // Update the event document in Firestore
      await window.firebaseUpdateDoc(window.firebaseDoc(window.dbFirestore, "events", eventId), {
        participants: e.participants,
        updated_at: e.updated_at
      });
      console.log("Event participants updated in Firestore.");
    } catch (error) {
      console.error("Error updating event participants in Firestore:", error);
      setToast("Failed to register. Please try again.", false);
      return;
    }
  ...
  window.saveAnnouncement = async function(id) {
    const e = db.events.find(x=>x.id===id);
    const newAnnouncement = $('#announcement_text').value.trim();
    
    try {
      await window.firebaseUpdateDoc(window.firebaseDoc(window.dbFirestore, "events", id), {
        announcement: newAnnouncement
      });
      e.announcement = newAnnouncement; // Update local cache
      setToast('Announcement published');
    } catch (error) {
      console.error("Error saving announcement:", error);
      setToast("Failed to publish announcement.", false);
    } finally {
      $('#announceModal')?.close();
    }
  }
  ...
  window.saveEvent = async function(id){
    ...
    try {
      if(id){
        await window.firebaseUpdateDoc(window.firebaseDoc(window.dbFirestore, "events", id), eventData);
        // Update local cache
        Object.assign(db.events.find(x=>x.id===id), eventData);
        setToast('Event updated');
      } else {
        const newId = uid();
        const newEvent = { ...eventData, id: newId, organiser_email: session.email, is_published: false, announcement: '', participants: [], created_at: new Date().toISOString() };
        await window.firebaseSetDoc(window.firebaseDoc(window.dbFirestore, "events", newId), newEvent);
        db.events.push(newEvent); // Add to local cache
  ...
  window.togglePublish = async function(id){
    const e = db.events.find(x=>x.id===id);
    if (!e) return;
    const newPublishStatus = !e.is_published;
    try {
      await window.firebaseUpdateDoc(window.firebaseDoc(window.dbFirestore, "events", id), { is_published: newPublishStatus });
      e.is_published = newPublishStatus; // Update local cache
      render();
      setToast(newPublishStatus ? 'Event published!' : 'Event unpublished.');
    } catch (error) {
      console.error("Error toggling publish status:", error);
      setToast("Failed to change publish status.", false);
    }
  }
  ...
  window.deleteEvent = async function(id){
    ...
    try {
      await window.firebaseDeleteDoc(window.firebaseDoc(window.dbFirestore, "events", id));
      db.events.splice(db.events.findIndex(x=>x.id===id),1); // Update local cache
      render();
  ...
  function getExportRows(eventId){
    ...
    return (e.participants || []).flatMap(p => 
      (p.details && p.details.length > 0) ? p.details.map((d, i) => ({
        ...
        "Total Amount Paid": i === 0 ? p.amount : 0, // Only show total amount for the first participant in a team
  ...
  // Initialize Firestore data and then render
  async function initializeApp() {
    await seedFirestoreData(); // Ensure base data exists
    await loadUsersFromFirestore(); // Load users into local cache
    // Set up realtime sync for events
    load("events", events => {
      db.events = events;
      render();
    });
